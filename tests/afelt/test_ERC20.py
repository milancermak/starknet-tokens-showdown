...
from utils import (
    ...
    assert_event_emitted, contract_path, State, Account, calculate_gas_cost
)
...
# testing vars
RECIPIENT = 123
INIT_SUPPLY = 1000 #to_uint(1000)
AMOUNT = 200 #to_uint(200)
UINT_ONE = to_uint(1)
UINT_ZERO = to_uint(0)
...
def contract_classes():
    ...
    erc20_cls = get_contract_class('contracts/afelt/ERC20Mintable.cairo', is_path=True)
...
async def test_approve(erc20_factory):
    ...
    assert execution_info.result.remaining == 0
    ...
    print("felt test_approve")
    print(return_bool.call_info.execution_resources)
    print("Gas cost:", calculate_gas_cost(return_bool))
...
async def test_transfer(erc20_factory):
    ...
    assert execution_info.result.balance == 0
    ...
    print("felt test_transfer")
    print(return_bool.call_info.execution_resources)
    print("Gas cost:", calculate_gas_cost(return_bool))

    # check account balance
    execution_info = await erc20.balanceOf(account.contract_address).invoke()
    assert execution_info.result.balance == INIT_SUPPLY - AMOUNT
